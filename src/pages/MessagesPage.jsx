...
export default function MessagesPage() {
  ...
  // Obtener matches y chats al montar el componente
useEffect(() => {
  const fetchMatches = async () => {
    try {
      ...
      const fetchedMatches = response.data.map((item) => {
        ...
        const chat = item.chat || { id: null, messages: [] };

        // Determinar el nombre y ID del otro usuario
        ...
        return {
          ...
          age: otherUser.age || 0,
          location: otherUser.country || "Desconocido",
          lastActive: otherUser.lastActive ? format(new Date(otherUser.lastActive), "dd/MM/yyyy HH:mm") : "Desconocido",
          image: otherUser.photos?.[0] || "/placeholder.svg",
          compatibility: 90,
      ...
      response.data.forEach((item) => {
        if (item.chat) {
          fetchedConversations[item.match.id] = item.chat.messages.map((msg) => ({
            ...msg,
            isRead: msg.isRead || msg.senderId === userId, // Asegurar isRead
          }));
        }
      });
...
useEffect(() => {
  if (activeMatchId) {
    ...
    axios
      .patch(`http://localhost:3000/chats/${activeMatch.chatId}/messages/read`, { userId }, {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await axios.post(`http://localhost:3000/chats/${activeMatch.chatId}/messages`, {
        senderId: userId,
        content: message,
      });
      ...
      <div className="h-screen flex flex-col">
        <div className="flex-1 flex overflow-hidden">
                        ...
                        >
                          <div className="flex items-center">
                            <div className="relative">
                              ...
                              {match.lastActive === format(new Date(), "dd/MM/yyyy HH:mm") && (
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <h3 className="font-medium flex items-center">
                        {activeMatch.name}
                        {activeMatch.lastActive === format(new Date(), "dd/MM/yyyy HH:mm") && (
                  ...
                  {activeConversation.length > 0 ? (
                    activeConversation.map((msg, index) => {
                      ...
                      return (
                        <div
                          key={msg.id}
                          className={`flex-${isUser ? 'end' : 'start'}`}
                        >
                          {!isUser && showAvatar ? (
                            <Avatar className="h-8 w-8 mr-2 mt-1">
                              <AvatarImage src={activeMatch?.image} alt={activeMatch?.name} />
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center p-6">
                      ...
                      <h3 className="text-xl font-bold mb-2">New match with {activeMatch?.name}</h3>
                      <p className="text-gray-400 mb-6 max-w-md">
                        It's the perfect time to start a conversation. You share interests in{' '}
                        {activeMatch?.commonInterests?.join(', ') || 'none'}.
                      </p>
                      <Button
                        className="bg-blue-600 hover:bg-blue-700"
                        onClick={() =>
                          setMessage(
                            `Hi ${activeMatch?.name}! I really liked your profile, especially that you like ${
                              activeMatch?.commonInterests?.[0] || 'something special'
                            }.`
                          )
                        }
                      >
                ...
                <div className="p-4 border-t border-gray-800 bg-gray-900">
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="flex-1 relative">
                      ...
                      />
                      <Popover open={showEmojiPicker} onChange={setShowEmojiPicker}>
                        <PopoverTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon"
          ...
          {showProfileInfo && activeMatch && (
            ...
            <DialogContent className="bg-gray-100 text-black max-w-md">
              <DialogHeader>
                <DialogTitle>About {activeMatch.name}</DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                <div className="flex items-center space-x-4">
                  <Avatar className="h-16 w-16">
                    <AvatarImage source={activeMatch.image} alt={activeMatch.name} />
                    <AvatarFallbackImage>{activeMatch.name.charAt(0)}</AvatarFallbackImage>
                  </Avatar>
                  <div>
                    <p className="text-lg font-medium">
                      {activeMatch.name}, {activeMatch.age}
                    </p>
                    <p className="text-gray-500">{activeMatch.location}</p>
                  </div>
                </div>
                <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {activeMatch.commonInterests?.length > 0 ? (
                        activeMatch.commonInterests.map((interest,) => (
                          ...
                          <p className="text-sm text-gray-500">No common interests.</p>
                        )}
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="flex items-start space-x-2">
                        <Calendar className="h-5 w-0 mt-2 text-gray-400 mt-1" />
                        <p className="text-xs text-gray-500">Joined 2 months ago.</p>
                      </div>
                      <div className="flex items-start space-x-2">
                        <MapPin className="h-5 w-25 text-gray-400 mt-1" />
                        <p className="text-xs text-gray-500">5 miles away.</p>
                      </div>
                    </div>
                    <Button
